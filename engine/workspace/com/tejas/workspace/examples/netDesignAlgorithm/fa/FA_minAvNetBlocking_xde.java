...
/**
 * Given a network topology, the capacities in the links and the offered traffic, this algorithm obtains the traffic routing that minimizes the
 * average network blocking B, estimated according to the formula: \( B = \frac{1}{\sum_d h_d} \sum_e y_e B_e \), where \( h_d \) is the traffic offered
 * by demand \( d \) [in Erlangs], and \( B_e \) is the Erlang-B blocking for link \( e \), assuming that the offered traffic in the link
 * \( y_e \) is all the traffic routed to \( e \) (traffic does not shrink because of the blockings in the rest of the links).
 *
 * @author Pablo Pavon-Marino, Jose-Luis Izquierdo-Zaragoza
 * @version 1.0, March 2013
 */
public class FA_minAvNetBlocking_xde implements IAlgorithm
{
	@Override
...
//			op.addConstraint("y_e(" + e + ") == h_d * x_de(all," + e + ")");
	...
	public String getDescription()
	{
		...
		aux.append("Given a network topology, the capacities in the links and the offered traffic, this algorithm obtains the traffic routing that minimizes" +
				" the average network blocking B, estimated according to the formula: \\( B = \\frac{1}{\\sum_d h_d} \\sum_e y_e B_e \\), where \\( h_d \\) is the " +
				"traffic offered by demand \\( d \\) [in Erlangs], and \\( B_e \\) is the Erlang-B blocking for link \\( e \\), assuming that the offered traffic in" +
				" the link \\( y_e \\) is all the traffic routed to \\( e \\) (traffic does not shrink because of the blockings in the rest of the links).");
		return aux.toString();
	}
