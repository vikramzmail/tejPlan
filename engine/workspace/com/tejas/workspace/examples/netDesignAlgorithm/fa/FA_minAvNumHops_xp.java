 ...
 * the average number of hops using a flow-path formulation. For each demand, the set of admissible paths is composed of the ranking of (at most)
 ...
 */
public class FA_minAvNumHops_xp implements IAlgorithm
{
    @Override
	...
	op.addDecisionVariable("y_e", false , new int[] { 1 , E }, 0, Double.MAX_VALUE); // amount of traffic in each link
	...
	op.setObjectiveFunction("minimize", "sum(y_e) / sum(h_d)");
	...
	op.addConstraint("A_ep * (h_p .* x_p)' == y_e'"); // set the traffic in each link equal to y_e
	op.addConstraint("y_e <= u_e"); // the traffic in each link cannot exceed its capacity
	...
	return "Ok!: |P| = " + cpl.getNumberOfPaths() + ", num routes: " + netPlan.getNumberOfRoutes();
    }
